...
async function getList(pageRequested = 1) {
  const responseJson = await request(
    'POST',
    '/participant/paginated-find',
    {
      model: 'Project',
      query: {},
      page: pageRequested,
...
async function addListInfo(itemsInPage) {
...
async function jumpToPage(pageRequested = 1) {
  ...
  const { itemsInPage, numberOfItems } = await getList(pageRequested);
...
function ParticipantProjectList() {
  onMount(async () => {
    await checkSessionJwt();
    const { numberOfItems, itemsInPage } = await getList();
    await addListInfo(itemsInPage);
...
export default ParticipantProjectList;
