...
async function addListInfo() {
  ...
  itemsInPage?.forEach((item) => {
    ...
    // Status
    let statusInfo = '';
    if (item.status === USER_STATUS_PENDING_EMAIL_CONFIRMATION) {
      statusInfo =
        '<span class="text-blue-400">Aguardando confirmação de email</span>';
    } else if (item.status === USER_STATUS_EMAIL_CONFIRMED) {
      statusInfo = '<span class="text-green-400">Email confirmado</span>';
    }
    ...
    // Contents
    const itemInfo = `<div class="py-1">Nome: ${item?.name}<br />Estado: ${statusInfo}</div>`;
    const plus = `<div class="text-center text-purple-500 text-xl">+</div>`;
    el.innerHTML = `${itemInfo}${plus}`;

    // Add click listener
    el.addEventListener('click', async () => {
      window.location.href = `/app/${userType}/examiner/list/details?email=${item.email}`;
    });
...
export default AdminExaminerList;
